...
public class MergeSortThreeWay extends AbsSortAlgorithm{
	...
	private void merge (Comparable[] a, int lo, int left, int right, int hi) {
		...
		for (int m = lo; m <= hi; m++) {
			...
			if(left_exausted) {
				if(middle_exausted && !right_exausted) {a[m] = aux[k++];}
				else if(!middle_exausted && right_exausted) {a[m] = aux[j++];}
				else if  (less(aux[k], aux[j]))  { a[m] = aux[k++]; } 
				else                        { a[m] = aux[j++]; }
			}else if(middle_exausted) {
				if(right_exausted) {a[m] = aux[i++];}
				else if  (less(aux[k], aux[i]))  { a[m] = aux[k++]; } 
				else                        	{ a[m] = aux[i++]; }
